...
pcc (PreConfComponentInputs _lbc pbd _comp) =
  ...
  where
    autogenDir = buildDirPBD pbd </> unsafeMakeSymbolicPath "inner/autogen"
...
preBuildRules (PreBuildComponentInputs { buildingWhat = what, localBuildInfo = lbi, targetInfo = tgt }) = mdo
  let verbosityFlags = buildingWhatVerbosity what
      ...
      -- Check that this rule is demanded via the extra-bundled-libraries demand.
      -- No other rule demands it.
      runPpAction4 (PpInput {..}) = do
        let verbosity = mkVerbosity defaultVerbosityHandles verbosityFlags
      ...
      mkRule3 =
        staticRule
          ...
          [ ]
          ( Location autogenDir (unsafeMakeSymbolicPath "Gen2.c") NE.:| [] )

      mkRule4 =
        staticRule
          (mkCommand (static Dict) (static runPpAction4) $ PpInput {genDir = autogenDir, ..})
          [ ]
